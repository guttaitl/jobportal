...
def _serialize_job_list(j) -> dict:
...
def _serialize_job_detail(j) -> dict:
    """Full serialization for single-job views."""
    return {
        "job_id": j.jobid,
        "title": j.job_title,
        "company": j.client_name,
...
def get_recent_jobs(limit: int = Query(10, ge=1, le=50), db: Session = Depends(get_db)):
    ...
    return {
        "success": True,
        "jobs": [_serialize_job_list(j) for j in jobs],
        "total": len(jobs),
        "limit": limit,
    }
...
def get_job(job_id: str, db: Session = Depends(get_db)):
    ...
    return {"success": True, "job": _serialize_job_detail(job)}
...
def create_job(
    ...
    """Create a new job posting. AI enrichment and email run in the background."""
    ...
    # Non-blocking AI + email notification
    background_tasks.add_task(_handle_job_email, job, job_id)
...
def _handle_job_email(job: JobBase, job_id: str) -> None:
    """Generate AI content and send job notification email."""
    try:
        ...
        logger.error(f"Background email task failed for job {job_id}: {e}")